...
void CubeMapApp::Draw(const GameTimer& gt)
{
	...
	//  材质数据 绑定为 着色器资源，可以在shader中作为 结构化缓冲区（动态索引）
	...
	// 根参数3 描述符表 ：绑定 天空球纹理（它在 SRV堆 中 mSkyTexHeapIndex的位置/最后一个 ： 而且定义根参数的时候也设置了1）
	...
	// 根参数4 描述符表-5个srv：绑定srv堆（容量5）中的 srv（但是算上 天空球也只有4个srv？？） 
	mCommandList->SetGraphicsRootDescriptorTable(4, mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());

	// shader中 也只用到了 4个DiffuseMap（包括天空球）
	//			然后 又用一个 cubemap 存 根参数3的 天空球
...
void CubeMapApp::BuildRootSignature()
{
	...
	texTable0.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0, 0);	// cubemap的SRV

	CD3DX12_DESCRIPTOR_RANGE texTable1;
	texTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 5, 1, 0);	// 其余纹理的SRV
    ...
    slotRootParameter[2].InitAsShaderResourceView(0, 1);// 材质结构化缓冲区
	// 天空贴图纹理
...
{
	//
	// Create the SRV heap. （cubemap和其他的纹理 共用一个SRV堆）
	//							这里容量是5，但只有4个？
	//
	...
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBE;	// 这里使用的是 cubemap 
	...
	mSkyTexHeapIndex = 3;	// 环境贴图 在SRV堆中的索引
}
...
void CubeMapApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L".\\main18\\Cubemap\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L".\\main18\\Cubemap\\Shaders\\Default.hlsl", nullptr, "PS", "ps_5_1");
	
	mShaders["skyVS"] = d3dUtil::CompileShader(L".\\main18\\Cubemap\\Shaders\\Sky.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["skyPS"] = d3dUtil::CompileShader(L".\\main18\\Cubemap\\Shaders\\Sky.hlsl", nullptr, "PS", "ps_5_1");
...
void CubeMapApp::BuildPSOs()
{
	...
	skyPsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;			// 相机在球内部

	// Make sure the depth function is LESS_EQUAL and not just LESS.  
	// Otherwise, the normalized depth values at z = 1 (NDC) will 
	// fail the depth test if the depth buffer was cleared to 1.
	//	深度测试需要 小于等于 ；天空球z=1（最远处）不被裁剪
...
void CubeMapApp::BuildMaterials()
{
    ...
    sky->Name = "sky";
    sky->MatCBIndex = 4;			// 在材质buffer中的索引

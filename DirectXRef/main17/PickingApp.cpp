...
void PickingApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L".\\main17\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L".\\main17\\Shaders\\Default.hlsl", nullptr, "PS", "ps_5_1");
...
void PickingApp::BuildMaterials()
{
	...
	highlight0->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 0.0f, 0.3f);
	//highlight0->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 0.0f, 1.0f);		// 拾取的高亮 混合时 完全覆盖
...
void PickingApp::BuildRenderItems()
{
	...
	pickedRitem->IndexCount = 0;		// 对于 高亮：只需要一个三角形=3 （在pick时设置）
...
void PickingApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
    ...
    for(size_t i = 0; i < ritems.size(); ++i)
    {
		...
		if(ri->Visible == false)	// 这里使用了 visible 控制是否绘制
			continue;

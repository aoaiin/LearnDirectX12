...
void TreeBillboardsApp::Draw(const GameTimer& gt)
{
	...
	// 20面球体
	//mCommandList->SetPipelineState(mPSOs["geoSpheres"].Get());
	//DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::geoSphere]);
...
void TreeBillboardsApp::LoadTextures()
{
	...
	auto treeArrayTex = std::make_unique<Texture>();	// 公告牌所需要的 纹理
...
void TreeBillboardsApp::BuildDescriptorHeaps()
{
	...
	auto treeArrayTex = mTextures["treeArrayTex"]->Resource;	// 公告牌的纹理资源
	...
	auto desc = treeArrayTex->GetDesc();	// 获取纹理的描述
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DARRAY;	// 使用维度是 TEXTURE2DARRAY ：只能同类型/大小的纹理
	srvDesc.Format = treeArrayTex->GetDesc().Format;
	srvDesc.Texture2DArray.MostDetailedMip = 0;
	srvDesc.Texture2DArray.MipLevels = -1;
	srvDesc.Texture2DArray.FirstArraySlice = 0;
	srvDesc.Texture2DArray.ArraySize = treeArrayTex->GetDesc().DepthOrArraySize; //treeArrayTex纹理资源desc 获取：纹理数组长度
...
void TreeBillboardsApp::BuildShadersAndInputLayouts()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"Shaders\\Default12.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"Shaders\\Default12.hlsl", defines, "PS", "ps_5_0");
	mShaders["alphaTestedPS"] = d3dUtil::CompileShader(L"Shaders\\Default12.hlsl", alphaTestDefines, "PS", "ps_5_0");
	
	//mShaders["treeSpriteVS"] = d3dUtil::CompileShader(L"Shaders\\TreeSprite12.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["treeSpriteVS"] = d3dUtil::CompileShader(L"Shaders\\TreeSprite12.hlsl", nullptr, "VS6", "vs_5_0");
	mShaders["treeSpriteGS"] = d3dUtil::CompileShader(L"Shaders\\TreeSprite12.hlsl", nullptr, "GS", "gs_5_0");
	mShaders["treeSpritePS"] = d3dUtil::CompileShader(L"Shaders\\TreeSprite12.hlsl", alphaTestDefines, "PS", "ps_5_0");

	mShaders["SphereVS"] = d3dUtil::CompileShader(L"Shaders\\Default12xifen.hlsl", nullptr, "VS", "vs_5_0");
	//mShaders["SphereGS"] = d3dUtil::CompileShader(L"Shaders\\Default12xifen.hlsl", nullptr, "GS2", "gs_5_0");
	mShaders["SphereGS"] = d3dUtil::CompileShader(L"Shaders\\Default12xifen.hlsl", nullptr, "GS0", "gs_5_0");
	mShaders["SpherePS"] = d3dUtil::CompileShader(L"Shaders\\Default12xifen.hlsl", nullptr, "PS", "ps_5_0");

	// 习题：法线顶点可视化
	mShaders["VNormalGS"] = d3dUtil::CompileShader(L"Shaders\\normal.hlsl", nullptr, "GSFNormal", "gs_5_0");
	//mShaders["VNormalGS"] = d3dUtil::CompileShader(L"Shaders\\normal.hlsl", nullptr, "GSVNormal", "gs_5_0");
	mShaders["VNormalVS"] = d3dUtil::CompileShader(L"Shaders\\normal.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["VNormalPS"] = d3dUtil::CompileShader(L"Shaders\\normal.hlsl", nullptr, "PS", "ps_5_0");
...
void TreeBillboardsApp::BuildGeoSphere() {
	GeometryGenerator geoGen;
	GeometryGenerator::MeshData geoSphere = geoGen.CreateGeosphere(0.8f,0);
...
void TreeBillboardsApp::BuildTreeSpritesGeometry()
{
	...
	static const int treeCount = 16;	// 公告牌数量
	// 创建顶点数据
...
void TreeBillboardsApp::BuildPSOs()
{
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC geoSpherePsoDesc = opaquePsoDesc;
	geoSpherePsoDesc.VS =
	{
	...
	//geoSpherePsoDesc.InputLayout = { mTreeSpriteInputLayout.data(), (UINT)mTreeSpriteInputLayout.size() };
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&geoSpherePsoDesc, IID_PPV_ARGS(&mPSOs["geoSpheres"])));


	// 法线可视化的PSO
	D3D12_GRAPHICS_PIPELINE_STATE_DESC NormalPsoDesc = geoSpherePsoDesc;
	NormalPsoDesc.VS =
	{
		reinterpret_cast<BYTE*>(mShaders["VNormalVS"]->GetBufferPointer()),
		mShaders["VNormalVS"]->GetBufferSize()
	};
	NormalPsoDesc.GS = {
		reinterpret_cast<BYTE*>(mShaders["VNormalGS"]->GetBufferPointer()),
		mShaders["VNormalGS"]->GetBufferSize()
	};
	NormalPsoDesc.PS =
	{
		reinterpret_cast<BYTE*>(mShaders["VNormalPS"]->GetBufferPointer()),
		mShaders["VNormalPS"]->GetBufferSize()
	};
	NormalPsoDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;	// 面的法线可视化
	//NormalPsoDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_POINT;	// 顶点法线可视化
	...
	//// 设置 输入布局
	//geoSpherePsoDesc.InputLayout = { mStdInputLayout.data(), (UINT)mStdInputLayout.size() };
	//ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&geoSpherePsoDesc, IID_PPV_ARGS(&mPSOs["geoSpheres"])));
...
void TreeBillboardsApp::BuildMaterials()
{
	...
	treeSprites->MatCBIndex = 3;			// 该材质在CB中的位置
	treeSprites->DiffuseSrvHeapIndex = 3;	// 该材质使用的纹理的索引
...
void TreeBillboardsApp::BuildRenderItems()
{
	...
	geoSphereRitem->ObjCBIndex = 4;//floor常量数据（world矩阵）在objConstantBuffer索引1上
	...
	XMStoreFloat4x4(&geoSphereRitem->TexTransform, DirectX::XMMatrixScaling(1.0f, 1.0f, 1.0f));
	...
	*normalRitem = *geoSphereRitem;	// 顶点的数据还是 球体的
	normalRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	//normalRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_POINTLIST;

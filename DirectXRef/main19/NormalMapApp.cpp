...
void NormalMapApp::UpdateMaterialBuffer(const GameTimer& gt)
{
	...
	for(auto& e : mMaterials)
	{
		...
		if(mat->NumFramesDirty > 0)
		{
			...
			matData.NormalMapIndex = mat->NormalSrvHeapIndex;	// 更新
...
void NormalMapApp::LoadTextures()
{
	...
	std::vector<std::wstring> texFilenames = 
	{
		...
		L"./Textures/default_nmap.dds",		// 法线
...
void NormalMapApp::BuildRootSignature()
{
	...
	texTable0.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0, 0);	// cubemap-1个：t0-space0

	CD3DX12_DESCRIPTOR_RANGE texTable1;
	texTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 10, 1, 0);	// 其他纹理-10个：t1-space0
    ...
    slotRootParameter[2].InitAsShaderResourceView(0, 1); //材质结构体（SRV： t0-space1）
	//  cubemap、其他纹理（看上面 描述符表中的设置 寄存器）
...
void NormalMapApp::BuildDescriptorHeaps()
{
	...
	for(UINT i = 0; i < (UINT)tex2DList.size(); ++i)	// 创建 列表中6个纹理的 SRV
	{
	...
	// 创建 静态cubemap/环境贴图 的 SRV
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBE;
	srvDesc.TextureCube.MostDetailedMip = 0;
	srvDesc.TextureCube.MipLevels = skyCubeMap->GetDesc().MipLevels;	// 环境贴图
...
void NormalMapApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L".\\main19\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L".\\main19\\Shaders\\Default.hlsl", nullptr, "PS", "ps_5_1");
	
	mShaders["skyVS"] = d3dUtil::CompileShader(L".\\main19\\Shaders\\Sky.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["skyPS"] = d3dUtil::CompileShader(L".\\main19\\Shaders\\Sky.hlsl", nullptr, "PS", "ps_5_1");
...
void NormalMapApp::BuildShapeGeometry()
{
	...
	for(size_t i = 0; i < box.Vertices.size(); ++i, ++k)
	{
		...
		vertices[k].Normal = box.Vertices[i].Normal;		// 法线信息
	...
	for(size_t i = 0; i < grid.Vertices.size(); ++i, ++k)
	{
		...
		vertices[k].Normal = grid.Vertices[i].Normal;		// 法线信息
...
void NormalMapApp::BuildMaterials()
{
	...
	bricks0->DiffuseSrvHeapIndex = 0;	// 见 build描述符堆，diffuse和normal一样
	bricks0->NormalSrvHeapIndex = 1;	// 材质结构体 中 ：添加一项 使用的法线贴图（同样在 SRV堆里）
	...
	sky->NormalSrvHeapIndex = 7;		// 这里怎么取了 7？，好像没看到有 环境贴图的normal

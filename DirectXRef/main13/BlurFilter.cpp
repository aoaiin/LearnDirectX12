...
void BlurFilter::Execute(ID3D12GraphicsCommandList* cmdList, 
						 ...
						 int blurCount)
{
	auto weights = CalcGaussWeights(2.5f);	// 计算一维方向，各个点的权值
	...
	for(int i = 0; i < blurCount; ++i)
	{
		...
		// 一行像素 需要 多少个组（设每组256个线程）= 宽度/一组处理个数
...
void BlurFilter::BuildResources()
{
	...
	ThrowIfFailed(md3dDevice->CreateCommittedResource(
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),	// 默认堆（不能被CPU读写，只能被GPU读写）

...
private:
	...
	ComPtr<ID3D12RootSignature> mPostProcessRootSignature = nullptr;	// 后处理的PSO（CS的PSO）
...
void BlurApp::OnResize()
{
	...
	{
		mBlurFilter->OnResize(mClientWidth, mClientHeight);	// 修改大小后，资源的大小、描述符都要重新创建
	}
}
...
void BlurApp::Draw(const GameTimer& gt)
{
	...
	// 执行 模糊类的函数(对buffer中的内容，使用CS进行模糊4次)
	//		传入Compute根签名，两个PSO是 computePSO
	...
	// 转换当前buffer的类型后，可以将计算结果，放入buffer中
...
void BlurApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"./main13/Shaders/Default.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L".\\main13\\Shaders\\Default.hlsl", defines, "PS", "ps_5_0");
	mShaders["alphaTestedPS"] = d3dUtil::CompileShader(L".\\main13\\Shaders\\Default.hlsl", alphaTestDefines, "PS", "ps_5_0");
	
	
	mShaders["horzBlurCS"] = d3dUtil::CompileShader(L".\\main13\\Shaders\\Blur.hlsl", nullptr, "HorzBlurCS", "cs_5_0");
	mShaders["vertBlurCS"] = d3dUtil::CompileShader(L".\\main13\\Shaders\\Blur.hlsl", nullptr, "VertBlurCS", "cs_5_0");

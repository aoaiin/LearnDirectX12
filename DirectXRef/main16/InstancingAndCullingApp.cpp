...
void InstancingAndCullingApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L".\\main16\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L".\\main16\\Shaders\\Default.hlsl", nullptr, "PS", "ps_5_1");
...
void InstancingAndCullingApp::BuildFrameResources()
{
    for(int i = 0; i < gNumFrameResources; ++i)
    {
		...
		// 这里 是不是应该是 渲染项的 instance数量（看buildRI中，所有的实例应该只有1个RI）
		mFrameResources.push_back(std::make_unique<FrameResource>(md3dDevice.Get(),
			1, (UINT)mAllRitems[0]->Instances.size(), (UINT)mMaterials.size()));
    }
}

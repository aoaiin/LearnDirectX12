...
void StencilApp::Draw(const GameTimer& gt)
{
	...
	// 标记 镜子的范围 ：通过模板测试，则使用 设置的 Ref 替换模板缓冲区中的值
	...
	// 绘制透明物体：改为原来的渲染pass，并设置模板Ref=0（不使用模板）（其实是做了，因为都是0，所以永远通过）
	// 
	...
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Transparent]);	// 绘制 透明/镜子


	// Draw shadows  绘制阴影
	// 习题4：取消 阴影的PSO
	mCommandList->SetPipelineState(mPSOs["shadow"].Get());	// 设置阴影的PSO：绘制一次后，模板值+1，不再通过模板测试
...
void StencilApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	// Don't let user move below ground plane.	物体移动（至少在 y=0上）
	...
	XMVECTOR toMainLight = -XMLoadFloat3(&mMainPassCB.Lights[0].Direction);	// 光向量 ：指向光源
	XMMATRIX S = XMMatrixShadow(shadowPlane, toMainLight);				// 利用 阴影矩阵 计算 点到阴影平面的投影

	XMMATRIX shadowOffsetY = XMMatrixTranslation(0.0f, 0.001f, 0.0f);	// 阴影向 上/y 偏移

	XMStoreFloat4x4(&mShadowedSkullRitem->World, skullWorld * S * shadowOffsetY);


	// 更新镜像矩阵
	//XMMATRIX floorWorld = XMMatrixTranslation(0.0f, 1.0f, 0.0f);
	//XMStoreFloat4x4(&mReflectedFloorRitem->World, floorWorld * R);
	XMStoreFloat4x4(&mReflectedFloorRitem->World, R);

	// 更新 镜面中的 物体的阴影 矩阵
	XMStoreFloat4x4(&mReflectedShadowedSkullRitem->World, skullWorld * S * shadowOffsetY * R);
...
void StencilApp::UpdateReflectedPassCB(const GameTimer& gt)
{
	...
	auto currPassCB = mCurrFrameResource->PassCB.get();
	currPassCB->CopyData(1, mReflectedPassCB);			// 镜像数据 存在 帧资源中的第二个passCB中
}
...
void StencilApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"Shaders\\Default11.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"Shaders\\Default11.hlsl", defines, "PS", "ps_5_0");
	mShaders["alphaTestedPS"] = d3dUtil::CompileShader(L"Shaders\\Default11.hlsl", alphaTestDefines, "PS", "ps_5_0");
...
void StencilApp::BuildPSOs()
{
	...
	//
	// PSO for marking stencil mirrors. 标记镜面
	//

	CD3DX12_BLEND_DESC mirrorBlendState(D3D12_DEFAULT);
	mirrorBlendState.RenderTarget[0].RenderTargetWriteMask = 0;	// 不写入颜色
	...
	mirrorDSS.FrontFace.StencilPassOp = D3D12_STENCIL_OP_REPLACE;	// 通过模板测试时，使用 ref 替换
	mirrorDSS.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;	// 永远通过模板测试
	...
	//
	// PSO for stencil reflections.	绘制 镜像物体
	//

	D3D12_DEPTH_STENCIL_DESC reflectionsDSS;
	reflectionsDSS.DepthEnable = true;
	reflectionsDSS.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ALL;	// 运行写入深度
	...
	reflectionsDSS.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;	// 模板测试 ：只有等于 才通过
	...
	// 镜面阴影物体PSO
	D3D12_GRAPHICS_PIPELINE_STATE_DESC reflectedShadowPsoDesc = drawReflectionsPsoDesc;
	reflectedShadowPsoDesc.DepthStencilState.FrontFace.StencilPassOp = D3D12_STENCIL_OP_INCR;// 深度模板测试通过，模板值递增
...
void StencilApp::BuildMaterials()
{
	...
	shadowMat->DiffuseSrvHeapIndex = 3;	// 阴影和实例 使用的是一个物体，材质和变换不同（使用的纹理index还是一样的）
	shadowMat->DiffuseAlbedo = XMFLOAT4(0.0f, 0.0f, 0.0f, 0.5f);	// 阴影颜色 + 与后台像素的混合
	//shadowMat->DiffuseAlbedo = XMFLOAT4(0.0f, 1.0f, 0.0f, 1.0f);	//
...
void StencilApp::BuildRenderItems()
{
	...
	*reflectedFloorRitem = *floorRitem;
	...
	//mShadowedSkullRitem = shadowedSkullRitem.get();
	mReflectedFloorRitem = reflectedFloorRitem.get();		// 暂存，之后要修改它的变换矩阵
